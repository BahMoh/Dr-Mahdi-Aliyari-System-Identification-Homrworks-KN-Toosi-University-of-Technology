...
%% 
u_n=u;
y_n=(y-min(y))/(max(y)-min(y));    %  Normal y =>    [0 1] 
 chioce_percent=0.8;
 Choice=find(u_n<=chioce_percent);
 u_ch=u(Choice);
 y_ch=y(Choice);
 rand_choice=randperm(length(Choice));
 u_ch=u_ch(rand_choice);
 y_ch=y_ch(rand_choice);

%% Divide Data
Q=100;
Q1=floor(length(u_ch)/10);
n=10;
...
for i=1:n
    ...
    ud{i}=u((i-1)*Q+1:(i*Q));
    yd{i}=y((i-1)*Q+1:(i*Q));
    ud_ch{i}=u_ch((i-1)*Q1+1:(i*Q1));
    yd_ch{i}=y_ch((i-1)*Q1+1:(i*Q1));
    %% Test & Train
    u_Train{i}=ud_ch{i}(1:ceil(Percent*length(ud{i})));
    u_Test{i}=ud{i}(ceil(Percent*length(ud{i}))+1:end);
    y_Train{i}=yd_ch{i}(1:ceil(Percent*length(yd{i})));
...
disp(sprintf('Error_local theta=%d',(Y-Y_hat)'*(Y-Y_hat)));
...
disp(sprintf('Error_mean theta=%d',(Y-Y_hat_m)'*(Y-Y_hat_m)));
...
grid on
title(sprintf('Error Bar : choice %s data from [0 0.8]',Switch{K}))
