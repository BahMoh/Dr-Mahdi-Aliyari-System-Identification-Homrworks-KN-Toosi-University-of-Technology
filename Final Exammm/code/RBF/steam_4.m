...
load steamgen
c=steamgen;
U=[c((87-7):(9600-7),2),c(87:9600,3),c(87:9600,4),c((87-1):(9600-1),5),c((1):(9600-86),5),c((87-9):(9600-9),6),c((87-6):(9600-6),7),c((87-49):(9600-49),8)];
T=c(87:9600,9)';
% num_traindata=8000;
% num_testdata=1600;
num_traindata=3500;
...
u(i,8)=(U(i,8)-min(U(:,8)))/(max(U(:,8))-min(U(:,8)));
...
for i=1:num_traindata
    for j=1:m
        ...
        t8(1,j)=((u(i,8)-c(8,j))/sigma8)^2;
        phi(1,j)=exp(-.5*(sqrt(t1(j)+t2(j)+t3(j)+t4(j)+t5(j)+t6(j)+t7(j)+t8(j))));
    ...
    e1(i)=z(i)-yhat(i);
      for j=1:m
        c(:,j)=c(:,j)+etha*e1(i)*w(j,1)*[(u(i,1)-c(1,j))/(sigma1^2);(u(i,2)-c(2,j))/(sigma2^2);((u(i,3)-c(3,j))/sigma3)^2;(u(i,4)-c(4,j))/(sigma4^2);(u(i,5)-c(5,j))/(sigma2^5);((u(i,6)-c(6,j))/sigma6)^2;((u(i,7)-c(7,j))/sigma7)^2;((u(i,8)-c(8,j))/sigma8)^2]*phi(j);
        ...
        sigma8=sigma8+etha1*e1(i)*w(j,1)*phi(j)*(1/(sigma8)^3)*((u(i,8)-c(8,j))^2);
      end 
   gama=p*xt/(1+xt'*p*xt);
    p=(eye(m)-gama*xt')*p;
    t(i)= trace(p);
...
for h=(num_traindata+1):num_testdata
     for g=1:m
     phi1(1,g)=exp(-.5*(sqrt(((u(h,1)-c(1,g))/sigma1)^2+((u(h,2)-c(2,g))/sigma2)^2+((u(h,3)-c(3,g))/sigma3)^2+((u(h,4)-c(4,g))/sigma4)^2+((u(h,5)-c(5,g))/sigma5)^2+((u(h,6)-c(6,g))/sigma6)^2+((u(h,7)-c(7,g))/sigma7)^2+((u(h,8)-c(8,g))/sigma8)^2)));
     end
        xt1=phi1';
    yhat(h)=xt1'*w;
    e2(h)=z(h)-yhat(h);
